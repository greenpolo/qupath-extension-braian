 ...
 * {@link qupath.imagej.detect.cells.WatershedCellDetection}
 * on a given image channel. It does so by leveraging {@link AbstractDetections}
 * interface
 */
public class ChannelDetections extends AbstractDetections {
     ...
     * @return the full image detection named as
     *         {@link ChannelDetections#FULL_IMAGE_DETECTIONS_NAME}
     */
        ...
        switch (fullImageAnnotations.size()) {
            ...
            default:
                throw new RuntimeException(
                        "There are multiple annotations called '" + FULL_IMAGE_DETECTIONS_NAME + "'. Delete them!");
        }
    }
     ...
     * It selects all detections that are computed in a container identified by
     * {@code channelName}.
     * 
     * @param channelName the name of the channel to which the detections are linked
     *                    to
     * @param hierarchy   where to find the detections
     * @throws NoCellContainersFoundException if no pre-computed detection was found
    ...
    /**
     * Creates an instance based on pre-computed cell detections.
     * Convenience constructor for scripts or headless mode.
     * 
     * @param channelName the name of the channel to which the detections are linked
     *                    to
     * @param hierarchy   where to find the detections
    ...
    public ChannelDetections(String channelName,
        ...
        this(channelName, hierarchy, null, null);
    }
    ...
    /**
     * Computes the detections using
     * {@link qupath.imagej.detect.cells.WatershedCellDetection} algorithm inside
     * the given annotations
     * 
     * @param channel     the channel to which the detections are linked to
     * @param annotations the annotations inside of which to compute the detections.
     *                    If null or empty, it will compute them on the whole image
     * @param config      parameters to give to the
     *                    {@link qupath.imagej.detect.cells.WatershedCellDetection}
    ...
    public ChannelDetections(ImageChannelTools channel,
        ...
        if (annotations == null) {
            ...
            throw new IllegalArgumentException(
                    "You must give at least one annotation on which to compute the detections");
        }
        Map<String, ?> params = config.build(channel);
        // TODO: check if the given annotations overlap. If they do, throw an error as
        // that would duplicate detections
     ...
     * 
     * @param channel    the channel to which the detections are linked to
     * @param annotation the annotation inside of which to compute the detections.
     *                   If null, it will compute them on the whole image
     * @param config     parameters to give to the
     *                   {@link qupath.imagej.detect.cells.WatershedCellDetection}
     * @param hierarchy  where to compute the detections
     * @throws NoCellContainersFoundException
    ...
    private static PathAnnotationObject compute(PathAnnotationObject container,
        ...
        try {
            ...
            ChannelDetections.getChildrenDetections(container)
                    .forEach(detection -> detection.setPathClass(container.getPathClass()));
            return container;
        } catch (InterruptedException e) {
            BraiAnExtension.logger.warn(
                    "Watershed cell detection interrupted. Returning empty list of detections for " + container + "!");
            return container;
        }
    }
